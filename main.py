...
def counter():
    my_file = open("resources/counter.txt", "r+")
...
def export_janus_tweets(x, y):
    ...
    the_counter = counter()
    my_best_tweets.to_csv('tweet_lists/tweets' + str(the_counter) + '.csv',
                          index=False)
    return the_counter
...
def tweet_to_image(name, username, showFavsRt, show_date, tweet, tweet_timestamp, favs, retweets, profile_image, tweet_id, media_url, r, g, b):
    ...
    if tweet_size[0] <= 900 and "\n" not in tweet:
        ...
        if tweet_size[0] <= 700:
            tweet_w = (width - 700) // 2
            tweet_h = (height - tweet_size[1] - media_offset_h) // 2 + 50
            draw.rectangle(((tweet_w - 60, tweet_h - 300), (tweet_w + 700 + 60,tweet_h + tweet_size[1] + media_offset_h + 200)),
                           fill="white")
